...
if($requestType=="mpd")
{
	processAndSendMPD($OriginalMPD,$PatchedMPD,$DASHContent,$offsetToNow);
	exit;
}

if($requestType=="seg")
{
	$segName=$requestType=$_GET['name'];
	corsHeader();
	header('Location: ' . $url);
	exit;
}
...
function updateTemplate($period)
{
	global $DASHContent;
				...
				if($node2->nodeName === 'Representation')
				{
					foreach ($node2->childNodes as $node3)
					{
						if($node3->nodeName === 'SegmentTemplate')
						{
							...
							$node3->setAttribute("initialization",$DASHContent . $init);
							
							$media = $node3->getAttribute("media");
							$node3->setAttribute("media",$DASHContent . $media);
						}
					}
				}
			}
